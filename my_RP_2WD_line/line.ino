...
void robot_BW_line(int sp, float kp)     /////////////////////////////-------------->>>คำสั่งภอยหลัง
  {
    lasts_time = millis();
        while(millis() - lasts_time < 350)
      {
...
void robot_TR(int sp, int offset)
  {
    ...
    */
    Motor(0, 0);delay(100);
    do{Motor(sp, -sp);}while(mcp_f(1)< md_sensors(1));delay(100);
    do{Motor(sp, -sp);}while(mcp_f(1)> md_sensors(1));
    do{Motor(sp, -sp);}while(mcp_f(0)> md_sensors(0));
...
void robot_TL(int sp, int offset)
  {
    ...
    */
    Motor(0, 0);delay(100);
    do{Motor(-sp, sp);}while(mcp_f(0)< md_sensors(0));delay(100);
    do{Motor(-sp, sp);}while(mcp_f(0)> md_sensors(0));
    do{Motor(-sp, sp);}while(mcp_f(1)> md_sensors(1));
    Motor(sp, -sp);delay(offset);
    Motor(0, 0);delay(100);
